...
class Cnn(nn.Module):
    def __init__(self, in_channels=1, num_classes=10):
        super().__init__()
        self.conv_1 = nn.Conv2d(in_channels, out_channels=64, kernel_size=3, stride=1, padding=0)
        self.conv_2 = nn.Conv2d(in_channels=64, out_channels=128, kernel_size=3, stride=2, padding=0)
        self.conv_3 = nn.Conv2d(in_channels=128, out_channels=256, kernel_size=3, stride=2, padding=0)
        # self.fc = nn.Sequential(
        #     nn.Linear(16*128, 256),
        #     nn.ReLU(),
        #     nn.Linear(256, num_classes)
        #     )
        self.conv_4 = nn.Conv2d(in_channels=256, out_channels=512, kernel_size=3, stride=2, padding=0)
        self.fc = nn.Sequential(
            nn.Linear(4 * 512, 256),
            nn.ReLU(),
            nn.Linear(256, num_classes)
...
class EcCnn_i(Cnn):
    def __init__(self, in_channels=1, num_classes=10, gtheta_cfg=[32, 32], *args, **kwargs):
        ...
        self.fc = nn.Sequential(
            nn.Linear(4*256 + gtheta_cfg[-1], 128),
            nn.ReLU(),
    ...
    def forward(self, x):
        ...
        x_1 = self.relu(self.ecc(x))         # ECLayr
...
class EcCnn(Cnn):
    def __init__(self, in_channels=1, num_classes=10, gtheta_cfg=[32, 32], *args, **kwargs):
        ...
        self.fc = nn.Sequential(
            nn.Linear(9*128 + 2*gtheta_cfg[-1], 256),
            nn.ReLU(),
            nn.Linear(256, num_classes)
            )
...
class EcCnnDTM_i(Cnn):
    def __init__(self, in_channels=1, num_classes=10, gtheta_cfg=[32, 32],
                 interval_1=[0.02, 0.28],
                 interval_2=[0.06, 0.29],
                 *args, **kwargs):
        super().__init__(in_channels, num_classes)
        ...
        self.fc = nn.Sequential(
            nn.Linear(9*128 + 2*gtheta_cfg[-1], 64),
            nn.ReLU(),
...
class EcCnnDTM(Cnn):
    def __init__(self, in_channels=1, num_classes=10, gtheta_cfg=[32, 32],
        ...
        self.ecc_1 = ECLayr(interval=interval_1, gtheta_cfg=gtheta_cfg, *args, **kwargs)
        self.ecc_2 = ECLayr(interval=interval_2, gtheta_cfg=gtheta_cfg, *args, **kwargs)
        self.fc = nn.Sequential(nn.Linear(784 + 3*gtheta_cfg[-1], 64),
                                nn.ReLU(),
                                nn.Linear(64, num_classes))
    ...
    def forward(self, x):
        ...
        x_1 = self.self.conv_layer(x)           # Cnn
        x_1 = self.flatten(x_1)

        x_2 = self.relu(self.ecc_1(x_dtm005))   # ECLayr 1
        x_3 = self.relu(self.ecc_2(x_dtm02))    # ECLayr 2

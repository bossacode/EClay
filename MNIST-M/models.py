...
class Cnn(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.conv1(x)           # output size: (64, 30, 30)
        x = self.pool(F.relu(x))    # output size: (64, 15, 15)
        x = self.conv2(x)           # output size: (128, 13, 13)
        x = self.pool(F.relu(x))    # output size: (128, 6, 6)
        x = self.conv3(x)           # output size: (256, 5, 5)
        x = self.pool(F.relu(x))    # output size: (256, 2, 2)
        x = self.flatten(x)
        x = self.fc(x)
        return x
...
# Cnn + ECLayr
class EcCnn_i(Cnn):
    def __init__(self, in_channels=3, num_classes=10, gtheta_cfg=[32, 32], *args, **kwargs):
...
class EcCnn(Cnn):
    def __init__(self, in_channels=3, num_classes=10, size_1=[32, 32], size_2=[30, 30], gtheta_cfg=[32, 32], *args, **kwargs):
        super().__init__(in_channels, num_classes)
        ...
        self.fc = nn.Sequential(
            nn.Linear(4*256 + 2*gtheta_cfg[-1], 128),
            nn.ReLU(),
            nn.Linear(128, num_classes)
            )
...
class EcCnnDTM_i(Cnn):
    def __init__(self, in_channels=3, num_classes=10, gtheta_cfg=[32, 32],
...
class EcCnnDTM(Cnn):
    def __init__(self, in_channels=3, num_classes=10, gtheta_cfg=[32, 32],
        ...
        self.fc = nn.Sequential(nn.Linear(4*256 + 3*gtheta_cfg[-1], 128),
                                nn.ReLU(),
                                nn.Linear(128, num_classes))

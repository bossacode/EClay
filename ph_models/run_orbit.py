...
with open(f"configs/ORBIT5K/{args.model_flag}.yaml", "r") as f:
    cfg = yaml.load(f, yaml.FullLoader)


if __name__ == "__main__":
    nsim = 20                                           # number of simulations to run
    noise_prob_list = [0.0, 0.05, 0.1, 0.15, 0.2, 0.25] # noise probabilitieslities
    ...
    for p in noise_prob_list:
        ...
        data_dir = f"../ORBIT5K/dataset/{prob}/"    # base directory path to where data is loaded

        print("-"*30)
        print(f"Corruption & noise rate: {p}")
        print("-"*30)

        # weight_dir = f"./saved_weights/ORBIT5K/{args.model}/noise_prob/{prob}/" # directory path to save trained weights

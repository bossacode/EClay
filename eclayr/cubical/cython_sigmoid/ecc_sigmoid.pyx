...
cdef class SigEccBackbone:
    ...
    def __init__(self, t_const=True, size=[28, 28], interval=[0.02, 0.28], steps=32, lam=200.):
        """
        Args:
...
# T-construction
    ...
    def cal_ecc_topdim(self, object x, bint backprop):
        ...
        """
        cdef Py_ssize_t batch_size, num_channels, b, c, cell, pix, i, num_neighbors, num_min
        cdef float filt, dim, t, sig
            ...
            for c in range(num_channels):   # iterate over channel
                ...
                for cell in range(self.num_cells): # iterate over all cells in cubical complex
                    ...
                    for i in range(self.steps):
                        t = self.tseq[i]
                        sig = (-1.)**dim
                        ecc_view[b, c, i] += sig * (sigmoid(t-filt, self.lam))
                    ...
                    if backprop:
                        ...
                        if dim == 2:
                            ...
                            for i in range(self.steps):
                                t = self.tseq[i]
                                grad_local_view[b, c, pix, i] -= self.lam * sigmoid(t-filt, self.lam) * (1 - sigmoid(t-filt, self.lam))
                            ...
                            if num_neighbors == 1:                          # there is 1 neighbor square
                                ...
                                if sq_filt[0] > sq_filt[1]:    
                                    ...
                                    for i in range(2):
                                        ...
                                        for i in range(self.steps):
                                            t = self.tseq[i]
                                            grad_local_view[b, c, pix, i] += 0.5 * self.lam * sigmoid(t-filt, self.lam) * (1 - sigmoid(t-filt, self.lam))
                            free(neighbor_sq)
                        # vertex
                        else:
                            neighbor_sq = self._find_neighbor_sq(cell, dim, &num_neighbors) # neighbor_sq points at a C array containing index of neighbor squares
                            if num_neighbors == 1:                          # there is 1 neighbor square
                                ...
                                for i in range(num_min):
                                    ...
                                    for i in range(self.steps):
                                        t = self.tseq[i]
                                        grad_local_view[b, c, pix, i] -= 1./num_min * self.lam * sigmoid(t-filt, self.lam) * (1 - sigmoid(t-filt, self.lam))
                                free(sq)
                            free(neighbor_sq)
                # cumsum
    ...
    #     tseq = np.linspace(self.t_min, self.t_max, self.steps, dtype=dtype_float)
    #     cdef float[:] tseq_view = tseq
    ...
    #                 for i in range(self.steps):
    #                     ecc_view[b, c, i] += (-1)**dim * sigmoid(tseq[i] - filt, self.lamda)
    ...
    #                         for i in range(self.steps):
    #                             grad_local_view[b, c, pix, i] += sigmoid_grad(tseq[i] - filt, self.lamda)
    #                     # edge
    #                     elif dim == 1:
    ...
    #                             grad_local_view[b, c, pix, t] += 1./num_max
    #                         free(vtx)
    #                         free(neighbor_vtx)
    #             # cumsum
    #             for i in range(self.steps - 1):

...
cdef float sigmoid(float x, float lamda):
    return exp(lamda * x) / (1 + exp(lamda * x))


@cython.cdivision(True) # turn off checking for division by zero
cdef float sigmoid_grad(float x, float lamda):
    return -sigmoid(x, lamda) * (1 - sigmoid(x, lamda))


cdef class SigmoidEccBackbone:
    cdef Py_ssize_t w, grid_h, grid_w, num_cells, steps
    cdef float t_min, t_max, resolution, lower_bound, lamda
    cdef const float[:] dimension

    def __init__(self, t_const=True, size=[28, 28], interval=[0.02, 0.28], steps=32, lamda=200):
        """
        Args:
                        ...
                        # vertex
                            ...
                            if num_neighbors == 1:                          # there is 1 neighbor square
                                ...
                                for i in range(num_min):
                                    ...
                                    grad_local_view[b, c, pix, t] += 1./num_min
                                free(sq)
                            free(neighbor_sq)
                # cumsum
                for i in range(self.steps - 1):

...
from adprnet_train import CustomDataset, eval, AdPRNet18, run_name, len_cn, file_cn_list, x_dir_list, y_dir, ntimes
...
if __name__ == "__main__":
    ...
    for cn in range(len_cn):
        ...
        for n_sim in range(ntimes):
            ...
            print(f"Model: {AdPRNet18.__name__}")
            print("-"*30)
            weight_file = f"./saved_weights/{run_name}/{file_cn_list[cn]}/sim{n_sim+1}.pt"     # file path to trained model weights
            model = AdPRNet18().to(device)
            model.load_state_dict(torch.load(weight_file, map_location=device))
            test_loss, test_acc = eval(model, test_dataloader, loss_fn, device)

            if record_test_info:
                # save test information
                test_info[AdPRNet18.__name__].append({"sim" + str(n_sim+1):(test_acc, test_loss)})

            if record_tensorboard:
            # write to tensorboard
                writer = SummaryWriter(f"./runs/{run_name}/test/{file_cn_list[cn]}")
        ...
        if record_test_info:
            ...
            with open(test_info_dir + "/" + f"test_info.json", "w", encoding="utf-8") as f:
                json.dump(test_info[AdPRNet18.__name__], f, indent="\t")

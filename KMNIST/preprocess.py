...
def generate_data(n_train, n_val, noise_prob_list):
    ...
    ecc_dtm005 = CubECC2d(as_vertices=False, sublevel=True, size=[28, 28], interval=[0.03, 0.34, 32])
    pl_dtm005 = CubPL2d(superlevel=False, tseq=[0.03, 0.34, 32], K_max=2, dimensions=[0, 1])

    dtm02 = DTMLayer(m0=0.2, lims=[[-0.5, 0.5], [-0.5, 0.5]], size=[28, 28])
    ecc_dtm02 = CubECC2d(as_vertices=False, sublevel=True, size=[28, 28], interval=[0.06, 0.35, 32])
    pl_dtm02 = CubPL2d(superlevel=False, tseq=[0.06, 0.35, 32], K_max=2, dimensions=[0, 1])

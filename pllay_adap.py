...
class AdTopoLayer(nn.Module):
    def __init__(self, out_features, m0=0.05, T=50, lims=[[1,28], [1,28]], size=[28, 28], r=2, K_max=2, dimensions=[0, 1], device="cpu", filtration="dtm", robust=True):
        """
        Args:
            out_features: 
            T: 
        ...
        self.flatten = nn.Flatten()
        self.dtm_layer = DTMLayer(m0, lims, size, r, device)
    ...
    def forward(self, input):
        ...
        if self.filtration == "dtm":
            ...
            land, t_range = self.landscape_layer(x)

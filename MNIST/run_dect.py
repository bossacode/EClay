...
import os
import yaml
from train_test_dect import train_test_wandb, train_test
...
with open("configs/DECT.yaml", "r") as f:
    config = yaml.load(f, yaml.FullLoader)
config["device"] = "cuda" if torch.cuda.is_available() else "cpu"
config["model_params"]["device"] = "cuda" if torch.cuda.is_available() else "cpu"
...
if __name__ == "__main__":
    ...
    for n_train in n_train_list:
        project = f"MNIST_data_DECT"    # used as project name in wandb
        ...
        for p in noise_prob_list:
            ...
            weight_dir = f"./saved_weights/data_{n_train}/DECT/{noise_prob}/"    # directory path to save trained weights
            ...
            for n_sim in range(1, ntimes+1):
                ...
                train_test_wandb(EctCnnModel, config, train_dl, val_dl, test_dl, weight_path, True, False, project, group, job_type)
                # train_test(EctCnnModel, config, train_dl, val_dl, test_dl, weight_path)
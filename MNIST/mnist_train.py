...
class MNISTCustomDataset(Dataset):
    def __init__(self, x_dir, y_dir, train=True, random_seed=1):
        self.train = train
        x_train, x_test = torch.load(x_dir)
        y_train, y_test = torch.load(y_dir)
        self.x_tr, self.x_val, self.y_tr, self.y_val = train_test_split(x_train, y_train, test_size=0.1, shuffle=True,
...
class MNISTCNN_Pi(nn.Module):
    def __init__(self, out_features=32, kernel_size=3,):
        ...
        self.conv_layer = nn.Sequential(nn.Conv2d(1, out_features, kernel_size=3, padding=1),
                                        ...
                                        nn.ReLU(),
                                        nn.Flatten())
        self.topo_layer_1 = nn.Seqeuntial(nn.Flatten(),
        ...
        self.topo_layer_2 = nn.Sequential(nn.Flatten(),
                                          ...
                                          nn.ReLU())
        self.linear_layer = nn.Seqeuntial(nn.Linear(784+out_features+out_features, 64),
                                          nn.ReLU(),
                                          nn.Linear(64, 10),
                                          nn.Softmax(-1))
...
if __name__ == "__main__'":
    ...
    x_dir_list = ['mnist_x_' + file_cn_list[i] + '.pt' for i in range(len_cn)]

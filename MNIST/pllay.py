...
class CubPL2d(nn.Module):
    def __init__(self, as_vertices=True, sublevel=True, interval=[0.02, 0.28], steps=32, K_max=2, dimensions=[0, 1]):
        """_summary_
...
class PLLay(nn.Module):
    def __init__(self, as_vertices=True, sublevel=True, interval=[0.02, 0.28], steps=32, K_max=2, dimensions=[0, 1], in_channels=1, hidden_features=[32]):
        """
        Args:
            superlevel: 
            T: 

...
def gen_sampled_data(train_size_list, val_size=0.3, num_labels=10):
    ...
    """
    train_data = SVHN(root="./dataset/raw/", split="train", download=True, transform=ToTensor())  # shape: (73257, 3, 32, 32)
    test_data = SVHN(root="./dataset/raw/", split="test", download=True, transform=ToTensor())  # shape: (26032, 3, 32, 32)
    
    # normalize
    x_train = torch.from_numpy(train_data.data) / 255
    y_train = torch.from_numpy(train_data.labels)
    x_test = torch.from_numpy(test_data.data) / 255
    ...
    for train_size in train_size_list:
        ...
        x_tr_dtm005 = dtm_transform(x_tr, m0=0.05, lims=[[-0.5, 0.5], [-0.5, 0.5]], size=[32, 32])
        x_tr_dtm02 = dtm_transform(x_tr, m0=0.2, lims=[[-0.5, 0.5], [-0.5, 0.5]], size=[32, 32])

        # apply DTM on validation data
        x_val_dtm005 = dtm_transform(x_val, m0=0.05, lims=[[-0.5, 0.5], [-0.5, 0.5]], size=[32, 32])
        x_val_dtm02 = dtm_transform(x_val, m0=0.2, lims=[[-0.5, 0.5], [-0.5, 0.5]], size=[32, 32])
    ...
    x_test_dtm005 = dtm_transform(x_test, m0=0.05, lims=[[-0.5, 0.5], [-0.5, 0.5]], size=[32, 32])
    x_test_dtm02 = dtm_transform(x_test, m0=0.2, lims=[[-0.5, 0.5], [-0.5, 0.5]], size=[32, 32])
...
def gen_noise_data(cn_prob_list, dir_path):
    ...
    x_tr, _, _, y_tr = torch.load(dir_path + "/train.pt", weights_only=True)    # shape: (N_train, 3, 32, 32)
    x_val, _, _, y_val = torch.load(dir_path + "/val.pt", weights_only=True)    # shape: (N_val, 3, 32, 32)
    
    # load test data
    test_data = SVHN(root="./dataset/raw/", split="test", download=True, transform=ToTensor())  # shape: (26032, 32, 32)
